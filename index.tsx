...
/**
 * Affiliate Blog Engine - V7.6 (Restored abdouweb Edition)
 */

const STORAGE_KEY = 'abdouweb_storage_v1';
...
const DEFAULT_BLOG_DATA = {
    ...
    staticPages: {
        about: "مرحباً بكم في منصتي الشخصية abdouweb. أنا أركز على تمكين الفرد المغربي من خلال تزويده بأحدث أخبار التكنولوجيا، استراتيجيات تطوير الذات، ومراجعات دقيقة للمنتجات.",
        ...
        detailContainer.innerHTML = `
            <div class="bg-white dark:bg-gray-900 rounded-[2rem] border border-gray-100 dark:border-gray-800 overflow-hidden shadow-sm transition-colors">
                ...
                <div class="p-6 md:p-16">
                    ...
                    <div class="article-body text-gray-600 dark:text-gray-400 text-lg md:text-xl leading-relaxed mb-12 border-b border-gray-50 dark:border-gray-800 pb-12">
                        ${a.content.split('\n').map((p: string) => `<p>${p}</p>`).join('')}
                    </div>
                    
                    <div class="mt-8 text-center">
                        <h4 class="text-xl font-black mb-8 dark:text-white">تابعني وشارك المقال</h4>
                        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                            <a href="${state.social.fb}" target="_blank" class="flex flex-col items-center gap-3 p-6 bg-blue-600/10 text-blue-600 rounded-3xl hover:bg-blue-600 hover:text-white transition group shadow-sm">
                                ...
                                <span class="font-bold">فيسبوك</span>
                            </a>
                            <a href="${state.social.ig}" target="_blank" class="flex flex-col items-center gap-3 p-6 bg-pink-600/10 text-pink-600 rounded-3xl hover:bg-pink-600 hover:text-white transition shadow-sm">
                                ...
                                <span class="font-bold">انستغرام</span>
                            </a>
                            <a href="${state.social.pin}" target="_blank" class="flex flex-col items-center gap-3 p-6 bg-red-600/10 text-red-600 rounded-3xl hover:bg-red-600 hover:text-white transition shadow-sm">
                                <svg width="32" height="32" viewBox="0 0 24 24" fill="currentColor"><path d="M12.017 0C5.396 0 .029 5.367.029 11.987c0 5.079 3.158 9.417 7.618 11.162-.105-.949-.199-2.403.041-3.439.219-.937 1.406-5.966 1.406-5.966s-.359-.72-.359-1.781c0-1.668.967-2.914 2.171-2.914 1.023 0 1.518.769 1.518 1.69 0 1.029-.655 2.568-.994 3.995-.283 1.194.599 2.169 1.777 2.169 2.133 0 3.772-2.249 3.772-5.495 0-2.873-2.064-4.882-5.012-4.882-3.414 0-5.418 2.561-5.418 5.207 0 1.031.397 2.138.893 2.738.098.119.112.224.083.345l-.333 1.36c-.053.22-.174.267-.402.161-1.499-.698-2.436-2.889-2.436-4.649 0-3.785 2.75-7.261 7.929-7.261 4.162 0 7.398 2.967 7.398 6.931 0 4.136-2.607 7.464-6.227 7.464-1.216 0-2.359-.631-2.75-1.378l-.748 2.853c-.271 1.043-1.002 2.35-1.492 3.146 1.124.347 2.317.535 3.554.535 6.607 0 11.985-5.36 11.985-11.987C23.97 5.39 18.592.026 11.985.026L12.017 0z"/></svg>
                                <span class="font-bold">بنتريست</span>
                            </a>
...
function renderApp() {
            ...
            <article class="bg-white dark:bg-gray-900 rounded-[2rem] overflow-hidden border border-gray-100 dark:border-gray-800 flex flex-col h-full shadow-sm hover:shadow-xl transition group">
                ...
                <div class="p-8 flex flex-col flex-grow">
                    ...
                    <div class="flex items-center justify-between mt-auto pt-6 border-t border-gray-50 dark:border-gray-800">
                        ...
                        <a href="${o.url}" target="_blank" class="bg-gray-900 dark:bg-orange-600 text-white px-8 py-4 rounded-2xl font-black shadow-lg">شراء</a>
                    </div>
                </div>
            </article>
        `).join('');
            ...
            <article class="bg-white dark:bg-gray-900 p-4 md:p-6 rounded-[2rem] border border-gray-100 dark:border-gray-800 flex flex-col sm:flex-row gap-6 shadow-sm hover:shadow-lg transition group">
                ...
                <div class="flex-grow flex flex-col justify-center">
                    <div class="flex items-center gap-3 mb-2">
                        <span class="text-orange-600 text-[10px] font-black border-2 border-orange-600 px-3 py-0.5 rounded-full">${a.category || 'تقنية'}</span>
    ...
    if (isLoggedIn) {
        ...
        if(oL) oL.innerHTML = state.offers.map((o:any)=>`<tr class="border-b dark:border-gray-800"><td class="py-4 font-bold">${o.title}</td><td class="py-4 text-orange-600 font-bold">${o.price}</td><td class="py-4 text-left"><button onclick="window.deleteOffer('${o.id}')" class="bg-red-500 text-white px-4 py-1 rounded-lg text-xs font-bold">حذف</button></td></tr>`).join('');
        const aL = document.getElementById('admin-articles-list');
        if(aL) aL.innerHTML = state.articles.map((a:any)=>`<div class="flex justify-between items-center p-4 bg-gray-50 dark:bg-gray-800 rounded-2xl mb-2 font-bold"><span>${a.title}</span><button onclick="window.deleteArticle('${a.id}')" class="bg-red-500 text-white px-4 py-1 rounded-lg text-xs">حذف</button></div>`).join('');
    }
}
